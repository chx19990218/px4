...
//	if ((state==JS_CTRL && odom_data.p(2)>0.35&& js_ctrl_mode==JS_CTRL_MODE_FEEDBACK&& idling_state==NOIDLING) // JS_CTRL hovering case
...
//					param.config_full_thrust(hov_thr_kf.get_hov_thr());
					param.config_full_thrust(param.hov_per);

					hov_thr_kf.process(u.thrust);
					hov_thr_kf.publish_thr();
					
					
					double yaw_curr = get_yaw_from_quaternion(odom_data.q);
					Matrix3d wRc = rotz(yaw_curr);
					Matrix3d cRw = wRc.transpose();
					hov_thr_kf.update_hor_k(cRw, des.v, odom_data.v);
		#else			
...
//	if (idling_state == NOIDLING)
//	{
//		if ((state==JS_CTRL && odom_data.p(2)>0.2 && js_ctrl_mode==JS_CTRL_MODE_FEEDBACK) // JS_CTRL hovering case
//			|| (state==CMD_HOVER) // CMD_HOVER case
//			|| (state==CMD_CTRL   // CMD_CTRL verticle stationary case
//				&& idling_state==NOIDLING // NOT in idling mode
//				&& std::fabs(cmd_data.v(2)) < param.hover.vert_velo_limit_for_update
...
//	Desired_State_t des;
	...
	des.a = cmd_data.a;
//	ROS_INFO("cmd_data:%lf,%lf,%lf:",des.p.x(), des.p.y(), desp.z());
	controller.update(des, odom_data, u);
...
//			des.yaw = yaw_add(get_yaw_from_odom(), des_dyaw);
			des.yaw = yaw_add(start_yaw, des_dyaw);
			break;
		case BREAK:
