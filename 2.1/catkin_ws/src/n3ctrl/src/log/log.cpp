...
//	string logPath = param.log_path.c_str();
  ...
  string log_p_x_path = folderPath + "/log_p_x.txt";
	...
	string log_q_path = folderPath + "/log_q.txt";
	string log_acc_x_path = folderPath + "/log_acc_x.txt";
	string log_acc_y_path = folderPath + "/log_acc_y.txt";
	string log_acc_z_path = folderPath + "/log_acc_z.txt";
	string log_odom_q_path = folderPath + "/log_odom_q.txt";
	string log_target_path = folderPath + "/log_target.txt";
	...
	flightlog.log_q.open(log_q_path,std::ios::app);
	flightlog.log_acc_x.open(log_acc_x_path,std::ios::app);
	flightlog.log_acc_y.open(log_acc_y_path,std::ios::app);
	flightlog.log_acc_z.open(log_acc_z_path,std::ios::app);
	flightlog.log_odom_q.open(log_odom_q_path,std::ios::app);
	flightlog.log_target.open(log_target_path,std::ios::app);


	
}

void Logger::record_log(const ros::Duration& now_time,const Desired_State_t& des,const Odom_Data_t& odom_data, 
	...
	flightlog.log_u_thr << rc_data.thr << "/" << u.thrust << endl;
	flightlog.log_hovper << hovper << "/" << param.hor_x_k << "/" << param.hor_y_k << "/" << param.hor_z_k << endl;
	flightlog.log_t << now_time.toSec() << endl;
	flightlog.log_q << imu_data.q.w() << "/" <<imu_data.q.x() << "/" << imu_data.q.y() << "/" << imu_data.q.z() << endl;
	flightlog.log_odom_q << odom_data.q.w() << "/" << odom_data.q.x() << "/" << odom_data.q.y() << "/" << odom_data.q.z() << endl;
	flightlog.log_acc_x << imu_data.a.x() << endl;
	flightlog.log_acc_y << imu_data.a.y() << endl;
	flightlog.log_acc_z << imu_data.a.z() << endl;
	flightlog.log_target << odom_data.target_p.x() << "/" << odom_data.target_v.x() << "/" << odom_data.target_p.y() << "/" << odom_data.target_v.y() << endl;
