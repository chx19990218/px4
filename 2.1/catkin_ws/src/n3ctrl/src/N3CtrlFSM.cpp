...
N3CtrlFSM::N3CtrlFSM(Parameter_t& param_, Controller& controller_, HovThrKF& hov_thr_kf_, Logger& log_):
	param(param_), controller(controller_), hov_thr_kf(hov_thr_kf_), log(log_)
{
	state = DIRECT_CTRL;
	axis_states[0] = FIX;
	axis_states[1] = FIX;
...
void N3CtrlFSM::process()
{
	...
	ros::Duration record_time = now_time-start_ctrl_time;
	log.record_log(record_time,des,odom_data,u,rc_data,imu_data,hov_thr_kf.get_hov_thr());
...
////								<< t->tm_min<<":"
////								<< t->tm_sec ;
////		createlog_flag = false;
////	}
//	flightlog.open("/home/chx/catkin_ws/log/flightlog(" + create_time.str() + ").txt",std::ios::app);
//}
//void N3CtrlFSM::record_log(const ros::Time& now_time)
//{
//	if (!flightlog.is_open())
//		ROS_INFO("open flightlog failed");
//	else
//	{
//		flightlog << now_time.toSec() << endl;
////		flightlog <<"------Flight Log------"<<endl
////							<<"time:"
////							<< t->tm_year+1900<<"."
////							<< t->tm_mon+1<<"."
////							<< t->tm_mday<<"--"
...
////							<< t->tm_min<<":"
////							<< t->tm_sec << endl;
////		flightlog << "state:" << state << "/"
////							<< "idling_state:" << idling_state <<endl;
////		flightlog << "work_mode:" << work_mode << "/"
////							<< "js_ctrl_mode:" <<js_ctrl_mode << endl;
////	  flightlog << "des pose:" << des.p.x() << "/" << des.p.y() << "/" << des.p.z() << endl
////							<< "odom pose:" << odom_data.p.x() << "/" << odom_data.p.y() << "/" << odom_data.p.z() << endl
////							<< "des velo:" << des.v.x() << "/" << des.v.y() << "/" << des.v.z() << endl
////							<< "odom velo:" << odom_data.v.x() << "/" << odom_data.v.y() << "/" << odom_data.v.z() << endl
////							<< "ctrl_roll:" << toDeg(u.roll) << "/" << "ctrl_pitch:" << toDeg(u.pitch) << "/" << "ctrl_yaw:" << toDeg(u.yaw) << "ctrl_thr:" << u.thrust <<endl
////							<< "des orie:" << des.yaw << endl
////							<< "odom orie:" << 2*std::atan2(odom_data.q.z(), odom_data.q.w()) / M_PI * 180.0 << endl
////							<< "odom quan:" <<	odom_data.q.w() << "/" << odom_data.q.x() << "/" << odom_data.q.y() << "/" << odom_data.q.z() <<endl
////							<< "fullthrust:" << param.full_thrust << "/" << "hov_percent" << hov_thr_kf.get_hov_thr() << endl;
////		flightlog << "rc_roll:" << rc_data.roll << "/" 
////							<< "rc_pitch:" << rc_data.pitch << "/" 
////							<< "rc_yaw:" << rc_data.yaw << "/"
////							<< "rc_thr:" << rc_data.thr << endl
////							<< "enter_api_command:" << rc_data.enter_api_mode << "/" 
////							<< "is_api_command:" <<	rc_data.is_api_mode << endl
////							<< "enter_command_mode:" << rc_data.enter_command_mode << "/"
////							<< "is_command_mode:" << rc_data.is_command_mode <<endl;
////		flightlog << "x_axis_state:"	<< axis_states[0] << "/"
////							<< "y_axis_state:" << axis_states[1] << "/"
////							<< "z_axis_state:" << axis_states[2] << "/"
////							<< "yaw_axis_state:" << axis_states[3] << endl;
//	}
//}
