...
//F_des
	...
	{	
		...
		Vector3d v_c = wRc.transpose() * odom.v;
		fx += param.hor_x_k * v_c.x() * param.mass;
		fy += param.hor_y_k * v_c.y() * param.mass;
//		fy += 0.0 * v_c.z() * param.mass;

//		fx += 1.6 * v_c.x() * param.mass;
//		fy += 1.6 * v_c.y() * param.mass;
//		fz += 1.6 * v_c.z() * param.mass;
...
//		Eigen::AngleAxisd rollAngle(AngleAxisd(u.roll,Vector3d::UnitX()));
//		Eigen::AngleAxisd pitchAngle(AngleAxisd(u.pitch,Vector3d::UnitY()));
//		Eigen::AngleAxisd yawAngle(AngleAxisd(u.yaw,Vector3d::UnitZ())); 
//		Eigen::Quaterniond quaternion = yawAngle*pitchAngle*rollAngle;
//		
//		wRb_odom = quaternion.toRotationMatrix();
//		z_b_curr = wRb_odom.col(2);
//		u1 = F_des.dot(z_b_curr);
//		u.thrust = u1 / param.full_thrust;
//		uav_utils::limit_range(u.thrust,0.1 ,1.0);
	}
