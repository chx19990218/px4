...
void Odom_Data_t::get_target(nav_msgs::OdometryConstPtr pMsg) {
    target_p.x() = pMsg->pose.pose.position.x;
    target_p.y() = pMsg->pose.pose.position.y;
    target_p.z() = pMsg->pose.pose.position.z;
    
    target_v.x() = pMsg->twist.twist.linear.x;
    target_v.y() = pMsg->twist.twist.linear.y;
    target_v.z() = pMsg->twist.twist.linear.z;
}
...
//		if(hor_cnt<horizon)
//		{
//			 a_h.col(hor_cnt) << msg.linear_acceleration.x, msg.linear_acceleration.y, msg.linear_acceleration.z;
//		}else
//		{
//			a_h.block(0,0,3,horizon-1) = a_h.block(0,1,3,horizon-1);
//			a_h.col(horizon-1) << msg.linear_acceleration.x, msg.linear_acceleration.y, msg.linear_acceleration.z;
//		}
//		hor_cnt++;
//		
//		a(0) = a_h.row(0).sum()/horizon;
//    a(1) = a_h.row(1).sum()/horizon;
//    a(2) = a_h.row(2).sum()/horizon;
//		ROS_INFO("acc_z,before:%lf,after:%lf",msg.linear_acceleration.z,a(2));
