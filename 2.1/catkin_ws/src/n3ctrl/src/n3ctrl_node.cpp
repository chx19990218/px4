...
int main(int argc, char* argv[]) {
    ...
    HovThrKF hov_thr_kf(param);
		Controller controller(param, hov_thr_kf);
		param.config_from_ros_handle(nh);
	  cout << "main log path:  " << param.js_ctrl_mode << endl;
		Logger log(param);



    N3CtrlFSM fsm(param, controller, hov_thr_kf, log);
    pFSM = &fsm;
    ...
    ros::Subscriber target_sub =
        nh.subscribe<nav_msgs::Odometry>("/uav0/target",
                                         1000,
                                         boost::bind(&Odom_Data_t::get_target, &fsm.odom_data, _1),
                                         ros::VoidConstPtr(),
                                         ros::TransportHints().tcpNoDelay());
...
//        nh.advertise<geometry_msgs::WrenchStamped>("ctrl_so3/thrust", 10);
    ...
    fsm.last_ctrl_time = ros::Time::now();
		fsm.start_ctrl_time = ros::Time::now();
		fsm.start_yaw = fsm.get_yaw_from_odom();
    // ---- process ----
    while (ros::ok()) {
        r.sleep();
        ros::spinOnce();
